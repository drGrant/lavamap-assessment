...
class CameraController {
  constructor (camera, controller, threeViewport, bounds) {
    ...
    this.viewport.addEventListener('mouseenter', this.startMouseCapture.bind(this))
    this.viewport.addEventListener('mouseleave', this.stopMouseCapture.bind(this))
    this.viewport.addEventListener('mousedown', this.startCameraControl.bind(this))
    this.viewport.addEventListener('mouseup', this.stopCameraControl.bind(this))
    this.viewport.addEventListener('wheel', this.zoomCamera.bind(this))
  ...
  startMouseCapture () {
    this.viewport.addEventListener('mousemove', this.setMousePosition.bind(this))
    this.viewport.focus()
  }
  ...
  stopMouseCapture () {
    this.isRotating = false
    this.delta = 0
    this.viewport.removeEventListener('mousemove', this.setMousePosition.bind(this))
    this.viewport.blur()
    this.forward = false
    this.backward = false
    this.left = false
  ...
  rotateCamera () {
    if (this.delta) {
      this.controller.rotation.y -= THREE.MathUtils.degToRad(this.delta * 0.015)
    }
  }
